...
//	cv::namedWindow(windowNameSrc, CV_WINDOW_AUTOSIZE);
	...
	{
		///std::cout << i+1 <<  rotationVectors[i] << std::endl;
		cv::Rodrigues(rotationVectors[i], rotationMatrix[i]);
		///std::cout << i+1 <<  rotationMatrix[i] << std::endl;
	...
	for (int i(0); i < translationVectors.size(); i++) {
		///std::cout << i+1 << translationVectors[i] << std::endl;
	}
	...
	//	cv::imwrite(fileName2, silhouette);			//resultImg OutputImgName
	//}
